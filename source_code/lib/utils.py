...
def print_label(label, nc =1,size = (64,64)):
    """retruns(prints) a label on a torch tensor.
    Parameters
    ----------
    ...
    if nc ==1:
        new_img = Image.new('L', size, color='black')
        draw = ImageDraw.Draw(new_img)

        draw.text(xy = (5,size[1]/2),text=label ,fill=(1))
    else:
        new_img = Image.new('RGB', size, color='black')
        draw = ImageDraw.Draw(new_img)

        draw.text(xy = (5,size[1]/2),text=label ,fill=(255,255,255))

    np_arr = np.array(new_img)
    torch_arr = torch.from_numpy(np_arr)
    if nc >1:
        torch_arr = torch_arr.unsqueeze(0).transpose(3,0).squeeze(-1)
    elif nc ==1: 
        torch_arr = torch_arr.unsqueeze(0)
    return torch_arr
def save_args(state, save, epoch):
    if not os.path.exists(save):
        os.makedirs(save)
    filename = os.path.join(save, 'args-%04d.pth' % epoch)
